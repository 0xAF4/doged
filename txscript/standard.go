...
import (
	...
	"github.com/0xaf4/doged/ltcutil/mweb/mw"
...
func expectedInputs(script []byte, class ScriptClass) int {
	...
	case MultiSigTy:
		...
		// the original litecoind bug where OP_CHECKMULTISIG pops an
...
func pubKeyHashToAddrs(hash []byte, params *chaincfg.Params) []ltcutil.Address {
	// Skip the pubkey hash if it's invalid for some reason.
	var addrs []ltcutil.Address
	addr, err := ltcutil.NewAddressPubKeyHash(hash, params)
	if err == nil {
...
func scriptHashToAddrs(hash []byte, params *chaincfg.Params) []ltcutil.Address {
	// Skip the hash if it's invalid for some reason.
	var addrs []ltcutil.Address
	addr, err := ltcutil.NewAddressScriptHashFromHash(hash, params)
	if err == nil {
		addrs = append(addrs, addr)
	}
	return addrs
...
func ExtractPkScriptAddrs(pkScript []byte,
	chainParams *chaincfg.Params) (ScriptClass, []ltcutil.Address, int, error) {
	...
	if data := extractPubKey(pkScript); data != nil {
		var addrs []ltcutil.Address
		addr, err := ltcutil.NewAddressPubKey(data, chainParams)
		if err == nil {
			addrs = append(addrs, addr)
		}
	...
	if details.valid {
		...
		addrs := make([]ltcutil.Address, 0, len(details.pubKeys))
		for _, pubkey := range details.pubKeys {
			addr, err := ltcutil.NewAddressPubKey(pubkey, chainParams)
			if err == nil {
				addrs = append(addrs, addr)
			}
		}
	...
	if hash := extractWitnessPubKeyHash(pkScript); hash != nil {
		var addrs []ltcutil.Address
		addr, err := ltcutil.NewAddressWitnessPubKeyHash(hash, chainParams)
		if err == nil {
			addrs = append(addrs, addr)
		}
