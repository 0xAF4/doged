...
var MainNetParams = Params{
	Name:        "mainnet",
	Net:         wire.MainNet, // Нужно убедиться что это правильный magic number для DOGE
	DefaultPort: "22556",
	DNSSeeds: []DNSSeed{
		{"seed.dogecoin.com", true},
		{"seed.mophides.com", true},
		{"seed.dglibrary.org", true},  // ИСПРАВЛЕНО: должно быть true для активных seeds
		{"seed.dogechain.info", true}, // ИСПРАВЛЕНО: должно быть true для активных seeds
		// Добавить больше актуальных seeds
		{"seed.multidoge.org", false},
		{"seed2.dogecoin.com", true},
	},
	...
	SubsidyReductionInterval: 100000,          // ИСПРАВЛЕНО: У Dogecoin нет халвинга, награда фиксированная 10k DOGE
	TargetTimespan:           time.Minute * 4, // ИСПРАВЛЕНО: 4 минуты (4 блока по 1 минуте)
	TargetTimePerBlock:       time.Minute * 1, // ИСПРАВЛЕНО: 1 минута, а не 2.5
	...
	Checkpoints: []Checkpoint{
		...
		// ДОБАВИТЬ более свежие checkpoints для актуальности
		{4609723, newHashFromStr("2c8632bbf2c23cf0071820e3fe8dcaf5a3d69da25b4b7b924a6dc46df70f0dde")}, // Примерный
	},

	// Consensus rule change deployments.
	RuleChangeActivationThreshold: 6048, // 75% of MinerConfirmationWindow
	MinerConfirmationWindow:       8064, // Правильно для DOGE
	Deployments: [DefinedDeployments]ConsensusDeployment{
		DeploymentTestDummy: {
			BitNumber: 28,
		...
		DeploymentCSV: {
			BitNumber: 0,
			DeploymentStarter: NewMedianTimeDeploymentStarter(
				time.Unix(1485561600, 0), // May 1st, 2016
			),
			DeploymentEnder: NewMedianTimeDeploymentEnder(
				time.Unix(1517356801, 0), // May 1st, 2017
			),
		},
		DeploymentSegwit: {
			BitNumber: 1,
			DeploymentStarter: NewMedianTimeDeploymentStarter(
				time.Unix(1485561600, 0), // November 15, 2016 UTC
			),
			DeploymentEnder: NewMedianTimeDeploymentEnder(
				time.Unix(1517356801, 0), // November 15, 2017 UTC.
			),
		},
		DeploymentTaproot: {
			BitNumber: 2,
	...
	Bech32HRPSegwit: "dc", // ИСПРАВЛЕНО: должно быть "dc" для Dogecoin, не "doge" (слишком длинное)

	// Address encoding magics - ЭТО ПРАВИЛЬНО ДЛЯ DOGECOIN
	PubKeyHashAddrID:        0x1e, // starts with D (правильно!)
	ScriptHashAddrID:        0x16, // starts with 9 или A (правильно!)
	PrivateKeyID:            0x9e, // starts with 6 (uncompressed) or Q (compressed)
	WitnessPubKeyHashAddrID: 0x06, // starts with dc1q
	WitnessScriptHashAddrID: 0x0A, // starts with dc1

	// BIP32 hierarchical deterministic extended key magics
	HDPrivateKeyID: [4]byte{0x02, 0xfa, 0xca, 0xfd}, // ИСПРАВЛЕНО: должно быть специфично для DOGE
	HDPublicKeyID:  [4]byte{0x02, 0xfa, 0xc3, 0x98}, // ИСПРАВЛЕНО: должно быть специфично для DOGE

	// BIP44 coin type used in the hierarchical deterministic path
	HDCoinType: 3, // ИСПРАВЛЕНО: Dogecoin использует coin type 3, а не 2
}
