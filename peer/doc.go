...
/*
Package peer provides a common base for creating and managing Litecoin network
peers.

# Overview

This package builds upon the wire package, which provides the fundamental
primitives necessary to speak the litecoin wire protocol, in order to simplify
...
A quick overview of the major features peer provides are as follows:
  ...
  - Provides a basic concurrent safe litecoin peer for handling litecoin
    communications via the peer-to-peer protocol
  - Full duplex reading and writing of litecoin protocol messages
  ...
  - Flexible peer configuration
    ...
    6. Ability to register callbacks for handling litecoin protocol messages
...
caller to specify things such as the user agent name and version, the litecoin
network to use, which services it supports, and callbacks to invoke when litecoin
...
In order to do anything useful with a peer, it is necessary to react to litecoin
...
For convenience, a callback hook for all of the currently supported litecoin
...
(https://pkg.go.dev/github.com/0xaf4/doged/wire#hdr-Bitcoin_Improvement_Proposals)
*/
package peer
