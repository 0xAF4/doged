...
func (cs *CoinSet) TotalValue() (value ltcutil.Amount) {
...
func satisfiesTargetValue(targetValue, minChange, totalValue ltcutil.Amount) bool {
...
type CoinSelector interface {
	CoinSelect(targetValue ltcutil.Amount, coins []Coin) (Coins, error)
}
...
type MinIndexCoinSelector struct {
	MaxInputs       int
	MinChangeAmount ltcutil.Amount
}

// CoinSelect will attempt to select coins using the algorithm described
// in the MinIndexCoinSelector struct.
func (s MinIndexCoinSelector) CoinSelect(targetValue ltcutil.Amount, coins []Coin) (Coins, error) {
	cs := NewCoinSet(nil)
...
func (s MinNumberCoinSelector) CoinSelect(targetValue ltcutil.Amount, coins []Coin) (Coins, error) {
...
func (s MaxValueAgeCoinSelector) CoinSelect(targetValue ltcutil.Amount, coins []Coin) (Coins, error) {
	sortedCoins := make([]Coin, 0, len(coins))
	sortedCoins = append(sortedCoins, coins...)
...
func (s MinPriorityCoinSelector) CoinSelect(targetValue ltcutil.Amount, coins []Coin) (Coins, error) {
...
// ltcutil.Tx, a specific outpoint index, and the number of confirmations
...
func (c *SimpleCoin) Value() ltcutil.Amount {
	return ltcutil.Amount(c.txOut().Value)
}
