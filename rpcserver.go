...
func messageToHex(msg wire.Message) (string, error) {
	...
	if err := msg.BtcEncode(&buf, maxProtocolVersion, wire.LatestEncoding); err != nil {
...
func handleCreateRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	for encodedAddr, amount := range c.Amounts {
		...
		if amount <= 0 || amount*ltcutil.SatoshiPerBitcoin > ltcutil.MaxSatoshi {
			return nil, &btcjson.RPCError{
...
func createTxRawResult(chainParams *chaincfg.Params, mtx *wire.MsgTx,
	...
	txReply := &btcjson.TxRawResult{
		...
		Vsize:    int32(mempool.GetTxVirtualSize(ltcutil.NewTx(mtx))),
		Weight:   int32(blockchain.GetTransactionWeight(ltcutil.NewTx(mtx))),
...
func handleDecodeScript(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	p2sh, err := ltcutil.NewAddressScriptHash(script, s.cfg.ChainParams)
	if err != nil {
...
func handleGetBlock(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	blk, err := ltcutil.NewBlockFromBytes(blkBytes)
	if err != nil {
...
func handleGetTxOut(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	txOutReply := &btcjson.GetTxOutResult{
		...
		Value:         ltcutil.Amount(value).ToBTC(),
...
func fetchMempoolTxnsForAddress(s *rpcServer, addr ltcutil.Address, numToSkip, numRequested uint32) ([]*ltcutil.Tx, uint32) {
...
func handleSearchRawTransactions(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	params := s.cfg.ChainParams
	addr, err := ltcutil.DecodeAddress(c.Address, params)
	if err != nil {
		return nil, &btcjson.RPCError{
			Code:    btcjson.ErrRPCInvalidAddressOrKey,
			Message: "Invalid address or key: " + err.Error(),
...
func handleSignMessageWithPrivKey(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	wif, err := ltcutil.DecodeWIF(c.PrivKey)
	if err != nil {
		message := "Invalid private key"
		switch err {
		case ltcutil.ErrMalformedPrivateKey:
			message = "Malformed private key"
		case ltcutil.ErrChecksumMismatch:
...
func handleValidateAddress(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	addr, err := ltcutil.DecodeAddress(c.Address, s.cfg.ChainParams)
	if err != nil {
...
func jsonAuthFail(w http.ResponseWriter) {
	w.Header().Add("WWW-Authenticate", `Basic realm="ltcd RPC"`)
...
type rpcserverSyncManager interface {
	...
	SubmitBlock(block *ltcutil.Block, flags blockchain.BehaviorFlags) (bool, error)
