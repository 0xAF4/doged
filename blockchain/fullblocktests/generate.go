...
func makeSpendableOutForTx(tx *wire.MsgTx, txOutIndex uint32) spendableOut {
	return spendableOut{
		...
		amount: ltcutil.Amount(tx.TxOut[txOutIndex].Value),
	}
}
...
func payToScriptHashScript(redeemScript []byte) []byte {
	redeemScriptHash := ltcutil.Hash160(redeemScript)
...
func calcMerkleRoot(txns []*wire.MsgTx) chainhash.Hash {
	...
	utilTxns := make([]*ltcutil.Tx, 0, len(txns))
	for _, tx := range txns {
		utilTxns = append(utilTxns, ltcutil.NewTx(tx))
	}
...
func additionalCoinbase(amount ltcutil.Amount) func(*wire.MsgBlock) {
...
func additionalSpendFee(fee ltcutil.Amount) func(*wire.MsgBlock) {
	return func(b *wire.MsgBlock) {
...
// tests.
func createSpendTx(spend *spendableOut, fee ltcutil.Amount) *wire.MsgTx {
...
func createSpendTxForTx(tx *wire.MsgTx, fee ltcutil.Amount) *wire.MsgTx {
